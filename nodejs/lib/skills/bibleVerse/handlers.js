...
var handlers = {
    'GetDailyBibleVerseIntent': function() {
        ...
        try {
            var body = '';
            var req = http.request(bibleVerseUrl, (response) => {
                console.log(`STATUS: ${response.statusCode}`);
                console.log(`HEADERS: ${JSON.stringify(response.headers)}`);

                response.on('data', (chunk) => {
                    console.log(`GetDailyBibleVerseIntent data: ${chunk}`);
                    body += chunk;
                });
                response.on('end', () => {
                    console.log(`GetDailyBibleVerseIntent end: ${body}`);
                    var r = JSON.parse(body).votd;
